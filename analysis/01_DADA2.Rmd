...
# Evaluate output
typeof(merged_ASVs) # list
length(merged_ASVs) # length 14
names(merged_ASVs)
```
...
dim(raw_ASV_table) # 9 1274
class(raw_ASV_table) # Matrix and array
...
###################################################
# TRIM THE ASVS
# Let's trim the ASVs to only be the right size, which is 317.
...
raw_ASV_table_trimmed <- 
  raw_ASV_table[,nchar(colnames(raw_ASV_table)) %in% 317]
...
dim(noChimeras_ASV_table) # 14 1764
...
write.table(asv_tab, "data/01_DADA2/ASV_counts.tsv", sep = "\t", 
            quote = FALSE, col.names = NA)
# Write count table with ASV sequence names
write.table(noChimeras_ASV_table, "data/01_DADA2/ASV_counts_withSeqNames.tsv", 
...
# Write the table 
write.table(asv_tax, "data/01_DADA2/ASV_taxonomy.tsv", sep = "\t", 
            quote = FALSE, col.names = NA)


# THIRD, let's save to a RData object 
# Each of these files will be used in the analysis/02_Taxonomic_Assignment
# RDS files load better than RData 
saveRDS(noChimeras_ASV_table, file = "data/01_DADA2/noChimeras_ASV_table.RDS")
saveRDS(asv_tab, file = "data/01_DADA2/ASV_counts.RDS")
...
saveRDS(track_counts_df, file = "data/01_DADA2/track_read_counts.RDS")
